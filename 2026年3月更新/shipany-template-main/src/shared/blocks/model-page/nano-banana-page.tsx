...
const RESOLUTION_TO_KIE: Record<string, string> = {
  sd:    '1K',
  hd2k:  '2K',
  uhd4k: '4K',
};
...
export function NanoBananaPage({ slug }: { slug: string }) {
  ...
  // ── Poll a single task, resolve when done ────────────────────────────────────
  ...
  const pollUntilDone = useCallback(
    (taskId: string): Promise<string | null> =>
      new Promise((resolve) => {
        const tick = async () => {
          try {
            const res  = await fetch(`/api/nano-banana/status/${taskId}`);
  ...
  const handleGenerate = useCallback(async () => {
    ...
    if (subMode === 'image-to-image') {
      const hasImage = uploads.some((u) => u.status === 'uploaded' && u.url);
      if (!hasImage) {
    ...
    try {
      ...
      // Create `count` tasks concurrently
      const taskIds = await Promise.all(
        Array.from({ length: count }, () =>
          fetch('/api/nano-banana/create', {
            method: 'POST',
            ...
            body: JSON.stringify({
              ...
              aspectRatio: aspect,
              resolution:  RESOLUTION_TO_KIE[resolution] ?? '1K',
            }),
          })
            .then((r) => r.json())
            .then((j) => {
  ...
  }, [user, subMode, uploads, prompt, count, aspect, resolution, stopPolling, pollUntilDone, setIsShowSignModal, zh]);
          ...
          <div className="grid grid-cols-1 gap-5 lg:grid-cols-2">
            ...
            <div ref={formPanelRef} className="bg-card border-foreground/5 rounded-xl border p-6 shadow-sm md:p-8">
              ...
              <div className="mt-6 space-y-2">
                ...
                {(() => {
                  const activeOpt = IMAGE_MODEL_OPTIONS.find((m) => m.slug === model) ?? IMAGE_MODEL_OPTIONS[1];
                  return (
              ...
              {/* Reference image upload — image-to-image mode only */}
              ...
              {/* Prompt */}
              <div className="mt-6 space-y-2">
                <Label htmlFor="nb-prompt">{L.prompt}</Label>
                <Textarea
                  id="nb-prompt"
                  value={prompt}
              ...
              {/* Aspect ratio — 5-column card grid */}
              <div className="mt-6 space-y-2">
              ...
              {/* Resolution — 3 horizontal buttons */}
              <div className="mt-6 space-y-2">
                  ...
                  <div className={cn(
                    ...
                    {results.map((url, i) => (
                      <div key={i} className="group relative overflow-hidden rounded-xl border border-foreground/10">
                        ...
                        <a
                          href={url}
                          download={`nano-banana-${Date.now()}-${i + 1}.jpg`}
                          target="_blank"
                          rel="noreferrer"
