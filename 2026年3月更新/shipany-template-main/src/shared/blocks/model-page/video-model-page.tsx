...
type Locale       = 'en' | 'zh';
type CreationMode = 'image' | 'text';
type ModelTier    = 'fast' | 'pro';
type StyleMode    = 'standard' | 'realistic' | 'wild';
type Bilingual    = { en: string; zh: string };

// ── Video model dropdown data (shared with SeedancePage) ─────────────────────
...
export function VideoModelPage({ slug }: { slug: string }) {
  ...
  // ── State (all unconditionally before early return) ───────────────────────
  ...
  const [selectedModel, setSelectedModel] = useState(slug);
  ...
  const styleTierBlock = (
    <div className="mt-4 grid grid-cols-2 gap-3">
      <div className="space-y-1.5">
        <p className="text-[11px] font-medium text-muted-foreground">{zh ? '模式' : 'Style'}</p>
      ...
      <div className="space-y-1.5">
        <p className="text-[11px] font-medium text-muted-foreground">{zh ? '模型' : 'Model'}</p>
        <div className="flex gap-1">
          {([
    ...
    <div className="mt-4 grid grid-cols-2 gap-3">
      <div className="space-y-1.5">
        <div className="flex items-center justify-between">
          <p className="text-[11px] font-medium text-muted-foreground">{zh ? '时长' : 'Duration'}</p>
      ...
      <div className="space-y-1.5">
        <p className="text-[11px] font-medium text-muted-foreground">{zh ? '分辨率' : 'Resolution'}</p>
  ...
  const aspectBlock = (
    <div className="mt-4 space-y-1.5">
      <p className="text-[11px] font-medium text-muted-foreground">{zh ? '画面比例' : 'Aspect ratio'}</p>
        ...
        {ASPECT_RATIOS.map((r) => {
          ...
          return (
            ...
            >
              ...
              <span className={cn('text-[11px] font-medium', active ? 'text-[#6366F1]' : 'text-muted-foreground')}>
                {r.value}
              </span>
            </button>
          );
  ...
  const generateBtn = (
    <div className="mt-5">
      {isCheckSign ? (
        <button disabled className="flex w-full cursor-not-allowed items-center justify-center gap-2 rounded-lg bg-[#6366F1]/60 py-3 text-sm font-semibold text-white">
  ...
  // Simple prompt box (used in both modes)
  const simplePrompt = (
    <div className="relative mt-4">
      <div className="relative rounded-lg border border-foreground/10 transition-colors hover:border-foreground/15 focus-within:border-[#6366F1]">
        <textarea
          value={prompt}
  ...
  // ── Active model for dropdown display ─────────────────────────────────────
  const activeModelOption =
    VIDEO_MODEL_OPTIONS.find((m) => m.slug === selectedModel) ?? VIDEO_MODEL_OPTIONS[0];
          ...
          <div className="grid grid-cols-1 gap-8 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
            ...
            <div className="bg-card border-foreground/5 rounded-xl border p-6 shadow-sm">
              ...
              <div className="mb-4 flex items-center gap-2">
                ...
                <span className="text-sm font-semibold">{zh ? '选择模型' : 'Select model'}</span>
              </div>
              ...
              <div ref={modelDropdownRef} className="relative">
                ...
                >
                  <span
                    ...
                    style={{ backgroundColor: activeModelOption.iconBg }}
                  >
                    <SmartIcon name={activeModelOption.icon} size={16} style={{ color: activeModelOption.iconColor }} />
                  </span>
                  <span className="flex-1 text-left text-sm font-medium">
                    {activeModelOption.name[locale] ?? activeModelOption.name.en}
                  </span>
                  {activeModelOption.badge && (
                    <span
                      className="rounded px-1.5 py-0.5 text-[9px] font-bold uppercase tracking-wide text-white"
                      style={{ backgroundColor: activeModelOption.badgeColor ?? '#6366F1' }}
                    >
                      {activeModelOption.badge}
                    </span>
                  )}
                ...
                <>
                  <div className="mt-4 flex items-center justify-between">
                    ...
                    <label className="flex cursor-pointer items-center gap-1.5">
                      ...
                      <Switch checked={tailFrame} onCheckedChange={setTailFrame} />
                    </label>
                  </div>
                  ...
                  >
                    ...
                    <p className="text-[11px] text-muted-foreground">
                      {zh ? 'JPG, PNG, WebP — 每张最大 10MB' : 'JPG, PNG, WebP — max 10 MB each'}
                    </p>
                  </div>
