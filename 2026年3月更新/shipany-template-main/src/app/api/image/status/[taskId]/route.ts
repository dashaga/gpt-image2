...
export const maxDuration = 30; // extra time for R2 download + upload
...
export async function GET(
  ...
  try {
    ...
    // Already saved to R2 — return stored URLs without re-querying kie.ai
    ...
    // ── Success: download from kie.ai → upload to R2 → persist ──────────────

    if (result.state === 'success' && result.resultUrls.length > 0) {
      const mediaType = (task?.mediaType ?? 'image') as 'image' | 'video';
    ...
    console.error('[image/status]', e);
    return respErr(e.message ?? 'failed to query task status');
  }
}
